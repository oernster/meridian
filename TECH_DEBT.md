...
The six modules originally named here are **done**. `_main_window_actions.py` had six duplicated wiring handlers collapsed into one documented helper and two duplicated rebind handlers into another; `install_ops.py`, `shortcuts.py`, `repair_ops.py`, `uninstall_ops.py` and `_header_fit.py` each now say what is being degraded and why, and `running_app.py` was documented alongside them. Several turned out not to be swallowing anything at all: the shortcut and bundle-swap handlers translate a spread of COM and filesystem failures into one actionable error, and the swap's outer handler exists to restore the previous installation before re-raising.

Nineteen remain, in modules the original scoping did not name: `worker.py` (6), `icons.py` (5), `apache2_license_text.py` (4), `lgpl3_license_text.py` (2), `app.py` (1) and `main_window.py` (1). They are recorded here rather than left out so the count cannot quietly shrink by redefinition. `worker.py` is the one that matters, since it is the thread the operations actually run on; the two licence-text modules and `icons.py` are resource loaders whose degradation is visibly cosmetic.
