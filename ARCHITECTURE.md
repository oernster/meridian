...
| No Python module or QML component under `meridian/` exceeds 400 lines; nor does any module under `tests/` or `installer/`; nor does any of them sit in the danger band of 381 to 399. Three QML files and one test module remain over the cap and are carried in `_LEGACY_OVER_LIMIT` as tracked debt; that set may only shrink, so an entry that no longer needs the allowance fails the suite. Both band bounds derive from the cap rather than being written as second literals, so they cannot drift apart. | `tests/structural/test_boundaries.py::test_all_source_files_within_line_limit`, `::test_no_source_file_sits_in_the_danger_band` and `::test_legacy_allowlist_has_no_stale_entries` |
...
| Every QML file compiles. The coverage gate reads Python only and `qmllint` cannot be turned into a gate here (unqualified-access warnings by the hundred, inherent to a context-property front end, and an exit code of 0 regardless), so compiling is the check that holds: it catches a syntax error, a property assigned on a type that has none or an unresolvable component, which is what extracting components can introduce. | `tests/ui/test_qml_compiles.py` |
| The discovery panel's keyboard focus ring closes across the three files it now spans. Each handover is a signal the composing file connects, and a missed connection compiles and leaves every component correct alone, so it is asserted with real key events through a real window rather than by inspection. | `tests/ui/test_discovery_focus_ring.py` |
...
meridian/
  ...
  ui/
    ...
    qml/
      ...
      FeedDiscovery.qml     Feed discovery drawer, composition only: holds the search state, the selection and the error text, wires the search bar to the results and both to the controller, and owns the bulk-subscribe confirmations. The two halves name nothing outside themselves, so every crossing (focus handover, search, cap, subscribe) passes through here as a signal
