...
| The Guide names every button on both bands beside the mark the button draws. A button added to either band with no Guide line fails the suite; so does a Guide line naming a mark that has no render. Both would otherwise be silent at run time: an unlisted picture or an empty box. | `tests/structural/test_guide_marks.py` and `tests/ui/test_help_menu.py::test_every_picture_in_the_guide_is_a_real_file` |
...
meridian/
  ...
  ui/
    ...
    qml/
      ...
      HeaderBar.qml         Header: Import, Export, Search, Manage on the left; the MMSP specification, the theme toggle and Help on the right, Help last because it is the one that is always in the same place. Help is a dropdown stop: pressing it (or Down) drops HelpMenu beneath it. The two licences left this bar for the foot: they state what is true of the application rather than acting on what is being read, which is what everything remaining here does. Every stop is a mark with a tooltip rather than a labelled button; the marks are files rather than emoji, because an emoji is drawn by whichever font the platform picks and the row came out different on each of the three the application ships to. Reports what was pressed rather than acting. firstFocusItem is the Import button; focusLast is Help; markSize is the one statement of how tall a mark is drawn, which the foot takes its own size from
      ...
      HelpMenu.qml          The Help button's menu: Guide, then About Meridian. The application has no menu bar, so Help drops this open, as Audio Deck's Help button does. Opens with its first entry focused; Up and Down walk the entries and wrap; Enter or Space chooses; Escape closes back onto the button; Tab and Right leave forward, Shift+Tab and Left leave backward. Reports the choice rather than acting, handing focus back to the button first so the dialog that opens returns focus there when it closes
      GuideDialog.qml       The Guide, ported from ClearBudget's How It Works: every button on both bands named beside the mark it actually draws, then the rules no screen shows (how often a feed is asked, what a filter does, what leaves the machine) and the keyboard. Opens on Close, never on the page; the page is a Tab stop only while it overflows, never takes a click and paints no ring. Wears an AutoScroller
      AboutDialog.qml       About dialog (keyboard: Enter/Escape closes), reached from the Help menu. Carries Check for Updates, the manual entry point, since the application has no menu bar
