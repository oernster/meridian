...
| No Python module or QML component under `meridian/` exceeds 400 lines; nor does any module under `tests/` or `installer/`; nor does any of them sit in the danger band of 381 to 399. Two QML files remain over the cap and are carried in `_LEGACY_OVER_LIMIT` as tracked debt; that set may only shrink, so an entry that no longer needs the allowance fails the suite. Both band bounds derive from the cap rather than being written as second literals, so they cannot drift apart. | `tests/structural/test_boundaries.py::test_all_source_files_within_line_limit`, `::test_no_source_file_sits_in_the_danger_band` and `::test_legacy_allowlist_has_no_stale_entries` |
...
meridian/
  ...
  ui/
    ...
    qml/
      ...
      FeedSidebar.qml       Select-all, the bulk remove button, the sort chips and the feed list. The active sort chip is not a tab stop, so stepping into or out of the chips searches past it; that search is two functions here rather than the six copies it was
...
tests/
  ...
  ui/
    ...
    window_stub.py          A hand-written controller with exactly the surface main.qml reaches for, plus the loader that builds the real window against it
    ...
    test_discovery_focus_ring.py, test_main_window_focus_ring.py
                            The two keyboard rings, driven with real key events through a real window
...
- Structural AST tests enforce layer boundaries; the 400-line size limit is enforced over QML, the test tree and the installer as well as the Python package, with an explicit allowlist that can only shrink; the danger band below the cap is enforced alongside it
