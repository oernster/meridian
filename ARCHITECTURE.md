...
| Every QML file compiles. The coverage gate reads Python only and `qmllint` cannot be turned into a gate here (198 unqualified-access warnings on one file, inherent to a context-property front end), so compiling is the check that holds: it catches a syntax error, a property assigned on a type that has none, and an unresolvable component, which is what extracting components can introduce. | `tests/ui/test_qml_compiles.py` |
